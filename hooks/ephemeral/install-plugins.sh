...
APP_MANIFEST_FOILE="$STEAMAPPDIR/appmanifest_730.acf"
...
if [ $(($(date +%s) - $(sed -n 's/.*"LastUpdated"\s*"\([0-9]*\)".*/\1/p' $APP_MANIFEST_FOILE))) -le 30 ]; then
	...
	echo "Timestamp is older than 30 seconds"
fi
